...
scanned in-browser · **Profile:** `demo-nadine-park`
...
## Deterministic fill map — Nadine Park
...
```json
{
  ...
  "profile_ref": "demo-nadine-park",
  "fields": {
    "profile[first_name]":        { "type": "text",  "required": true,  "value": "Nadine" },
    "profile[last_name]":         { "type": "text",  "required": true,  "value": "Park" },
    "profile[preferred_name]":    { "type": "text",  "required": false, "value": "Nadine" },
    "email":                      { "type": "email", "required": true,  "value": "nadine.aibuilder@gmail.com" },
    "profile[phones][]":          { "type": "tel",   "required": true,  "value": "+65 8800 1983" },
    "profile[current_employer]":  { "type": "text",  "required": false, "value": "Randamu Pte. Ltd." },
    "resume":                     { "type": "file",  "required": true,  "value": "data/resume-nadine-park.pdf" },
    ...
    "profile[education][new0][degree_measure][value]":         { "type": "text",   "required": true, "value": "4.99" },
    "profile[education][new0][degree_measure][highest_value]": { "type": "select", "required": true, "value": "5.0",
      "options": ["4.0","5.0","0-100"] },
    "profile[education][new0][from]": { "type": "date", "required": true, "value": "2023-08-01" },
    "profile[education][new0][to]":   { "type": "date", "required": true, "value": "2027-05-31" },
    ...
    "answers[][4147d252-83f9-4296-941e-ff3d4e0f6eff]": { "type": "checkbox", "required": false, "value": ["June - August", "September - December"],
    ...
    "profile[optional_links][]": { "type": "text", "required": false, "value": "https://linkedin.com/in/nadinep" }
  }
}
```

### Mapping rationale (honest-but-favorable)
- **Sponsorship = No** — Nadine is a Singapore citizen; job is posted in HK/SG. Removes the top auto-reject.
- **GPA 4.99 / 5.0** — NUS uses a 5.0 CAP; matches the locked profile fact directly.
...
- **Degree = Bachelors, Major = Computer Science** — from the profile's NUS BComp.
- **Timing = June–August + September–December** — her Aug–Dec 2026 availability spans both
  buckets; no single option covers it exactly.
...
## Follow-ups
- `data/resume-nadine-park.pdf` still needs generating (1-page PDF matching the profile).
