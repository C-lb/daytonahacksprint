  ...
  return (
    <section className="mt-4">
      ...
      <ul className="mt-1.5 space-y-1.5">
        {items.map((it) => (
          <li key={it} className="flex gap-2 text-sm leading-snug text-charcoal/85">
...
export function JobCard({ job, match }: { job: Job; match: number }) {
  return (
    ...
    >
      ...
      <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-5 pb-6 pt-4" data-scrollable>
        <p className="text-sm font-semibold uppercase tracking-wide text-coral">{job.company}</p>
        <h2 className="font-display mt-0.5 break-words text-2xl font-bold leading-tight text-charcoal">
          {job.title}
        </h2>
